...
Process[] ssmProcesses = Process.GetProcessesByName("PalworldServerManager");
if (ssmProcesses.Length != 0)
{
    Console.WriteLine("PalworldServerManager 正在运行，请关闭后再更新！");
    Console.ReadKey();
    Environment.Exit(2);
}
...
if (!File.Exists(Path.Combine(workingDir, "PalworldServerManager.exe")))
{
    Console.WriteLine("找不到主程序 PalworldServerManager.exe");
    Console.ReadKey();
    Environment.Exit(2);
}
...
try
{
    ...
    fileBytes = await httpClient.GetByteArrayAsync(@"https://github.com/aghosto/Palworld-Server-Manager/releases/latest/download/SSM.zip");
    Console.WriteLine("GitHub 下载成功！");
    downloadSuccess = true;

    // 获取 GitHub 最新版本号
    string apiUrl = "https://api.github.com/repos/aghosto/Palworld-Server-Manager/releases/latest";
    var request = new HttpRequestMessage(HttpMethod.Get, apiUrl);
    request.Headers.Add("User-Agent", "PalworldServerManager");
...
catch (Exception ex)
{
    ...
    try
    {
        string apiUrl = "https://gitee.com/api/v5/repos/aGHOSToZero/Palworld-Server-Manager/releases/latest";
...
string configFile = Path.Combine(workingDir, "PSMSettings.json");
...
Console.ReadKey();
Process.Start(Path.Combine(workingDir, "PalworldServerManager.exe"));