...
public class MainSettings : PropertyChangedBase
{
    ...
    {
        string dir = Directory.GetCurrentDirectory() + @"\PSMSettings.json";
        string SettingsJSON = JsonSerializer.Serialize(settings, _jsonOptions);
        File.WriteAllText(dir, SettingsJSON);
}

public class RESTServerSettings : PropertyChangedBase
{
...
/// <summary>
    ...
    public static MainSettings LoadManagerSettings()
    {
        ...
        else
        {
            ...
            {
                Content = $"未找到管理器配置文件(PSMSettings.json)，设置未能导入。",
                PrimaryButtonText = "是",
            };
            yesDialog.ShowAsync();
...
{
    private string _ssmServerName = "幻兽帕鲁服务器";
    ...
    public RESTServerSettings RestServerSettings { get; set; } = new RESTServerSettings();
